    ...
    <div className={`container mx-auto px-4 md:px-0 ${fitToScreen ? 'h-full flex flex-col gap-6' : 'space-y-6'}`}>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 shrink-0">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Quản lý sơ đồ phân cấp khoa, cơ sở</p>
        </div>
      ...
      {!treeData || treeData.length === 0 ? (
        <div className="flex flex-col items-center justify-center p-16 bg-white rounded-xl shadow-sm border text-center">
          ...
          <p className="text-gray-500 mb-6 max-w-md">Hãy tạo thành phần tổ chức gốc (Ví dụ: Tên trường) để bắt đầu xây dựng sơ đồ phân cấp.</p>
          <button 
