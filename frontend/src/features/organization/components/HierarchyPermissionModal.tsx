...
export default function HierarchyPermissionModal({ isOpen, onClose, hierarchyLevels }: HierarchyPermissionModalProps) {
    ...
    activeRoleLevels.forEach((levelVal, idx) => {
      ...
      ranks.forEach(rank => {
        ...
        if (actualRole) {
          ...
          if (rank === 0) {
            label = isTop ? (lvl.managerRoleLabel || 'HIỆU TRƯỞNG') : `TRƯỞNG ${lvl.unitTypeName}`
          } else if (rank === 1) {
            label = `PHÓ ${isTop ? (lvl.managerRoleLabel || 'HIỆU TRƯỞNG') : lvl.unitTypeName}`
          } else {
            label = 'GIẢNG VIÊN'
          }
        }
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      toast.success(`Đã cập nhật quyền hạn cho ${successCount} vai trò theo phân cấp thực tế của nhà trường.`)
      onClose()
    } catch (err) {
        ...
        <div className="p-5 md:p-10 border-t border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          <div className="flex flex-col-reverse md:flex-row gap-3 md:gap-4">
            ...
            >
              ...
              {isApplying ? 'Đang thiết lập...' : 'Xác nhận áp dụng cho toàn trường'}
            </button>
          </div>
        </div>
      </div>
